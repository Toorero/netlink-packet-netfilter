...
use netlink_packet_core::{
    ...
    DefaultNla, Emitable, ErrorContext as _, Nla, NlaBuffer, Parseable,
...
impl Nla for SetAttribute {
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::Flags(bits) => emit_u32(buffer, bits.bits()).unwrap(),
            Self::KeyType(value)
            | Self::KeyLen(value)
            | Self::DataType(value)
            | Self::DataLen(value)
            ...
            | Self::Count(value) => emit_u32(buffer, *value).unwrap(),
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            NFTA_SET_FLAGS => Self::Flags(SetFlags::from_bits_retain(
                parse_u32(payload).context("invalid NFTA_SET_FLAGS value")?,
            )),
            ...
            NFTA_SET_KEY_LEN => Self::KeyLen(
                parse_u32(payload).context("invalid NFTA_SET_KEY_LEN value")?,
            ),
            ...
            NFTA_SET_POLICY => Self::Policy(
                parse_u32(payload).context("invalid NFTA_SET_POLICY value")?,
            ),
            ...
            NFTA_SET_ID => Self::Id(
                parse_u32(payload).context("invalid NFTA_SET_ID value")?,
            ),
            NFTA_SET_TIMEOUT => Self::Timeout(
                parse_u64(payload).context("invalid NFTA_SET_TIMEOUT value")?,
            ),
            ...
            NFTA_SET_HANDLE => Self::Handle(
                parse_u64(payload).context("invalid NFTA_SET_HANDLE value")?,
            ),
            ...
            NFTA_SET_COUNT => Self::Count(
                parse_u32(payload).context("invalid NFTA_SET_COUNT value")?,
            ),
