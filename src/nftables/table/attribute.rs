    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            NFTA_TABLE_FLAGS => Self::Flags(TableFlags::from_bits_retain(
                parse_u32(payload).context("invalid NFTA_TABLE_FLAGS value")?,
            )),
            NFTA_TABLE_USE => Self::Use(
                parse_u32(payload).context("invalid NFTA_TABLE_USE value")?,
            ),
            ...
            NFTA_TABLE_OWNER => Self::Owner(
                parse_u32(payload).context("invalid NFTA_TABLE_OWNER value")?,
            ),
