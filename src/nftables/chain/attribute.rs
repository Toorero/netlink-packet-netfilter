...
use crate::nftables::chain::ChainFlags;
...
impl Nla for ChainAttribute {
    fn value_len(&self) -> usize {
        match self {
            ...
            Self::Handle(_) => 8,
            Self::Hook(attrs) | Self::Counter(attrs) => {
                attrs.as_slice().buffer_len()
            }
    ...
    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ...
            Self::Handle(value) => emit_u64(buffer, *value).unwrap(),
            Self::Hook(attrs) | Self::Counter(attrs) => {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            NFTA_CHAIN_USE => Self::Use(
                parse_u32(payload).context("invalid NFTA_CHAIN_USE value")?,
            ),
            ...
            NFTA_CHAIN_FLAGS => Self::Flags(ChainFlags::from_bits_retain(
                parse_u32(payload).context("invalid NFTA_CHAIN_FLAGS value")?,
            )),
            NFTA_CHAIN_ID => Self::Id(
                parse_u32(payload).context("invalid NFTA_CHAIN_ID value")?,
            ),
